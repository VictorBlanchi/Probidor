...
type request =
  ...
  | MovePawn of { player: int; dir: direction } 
    (** Move the pawn. *)
...
let decode_request (json : Yojson.Basic.t) : request =
  ...
  | "place-wall" -> 
      let player = json |> member "player" |> to_int in 
      let orient = json |> member "orient" |> to_string |> orientation_from_string in 
...
let encode_request (req : request) : Yojson.Basic.t = 
  ...
  | MovePawn i -> 
      `Assoc [ ("action", `String "move-pawn")
             ; ("player", `Int i.player)
             ; ("dir"   , `String (direction_to_string i.dir)) ]
  | PlaceWall i -> 
      `Assoc [ ("action", `String "place-wall")
             ; ("player", `Int i.player)
...
(** Raises [Yojson.Basic.Util.Type_error] on invalid input. *)
let decode_response (status_code : Cohttp.Code.status_code) (json : Yojson.Basic.t) : response =
  let open Yojson.Basic.Util in
