...
let tests = 
  [ Test.make 
      ~name:"wall_in_board_imp_pos_in_board"
      ~print:(Print.pair wall_print board_print)
      (Gen.pair wall_gen board_gen) 
      begin fun (wall, board) -> 
        assume (wall_in_board board wall);
        List.for_all begin fun (p1, p2) ->
          pos_in_board board p1 && pos_in_board board p2
        end (wall_edges wall)
      end 

  ; Test.make 
      ~name:"pos_in_board_imp_wall_in_board"
      ~print:(Print.pair wall_print board_print)
      (Gen.pair wall_gen board_gen) 
      begin fun (wall, board) -> 
        assume (List.for_all begin fun (p1, p2) ->
                  pos_in_board board p1 && pos_in_board board p2
                end (wall_edges wall));
  ...
  ; Test.make 
      ~name:"add_wall_not_in_board"
      ~print:(Print.pair wall_print board_print)
      (Gen.pair wall_gen board_gen) 
      begin fun (wall, board) -> 
        assume (not (wall_in_board board wall));
        try add_wall board wall; false
        with Wall_out_of_bounds -> true 
      end  

  ; Test.make 
      ~name:"remove_wall_not_in_board"
      ~print:(Print.pair wall_print board_print)
      (Gen.pair wall_gen board_gen) 
      begin fun (wall, board) -> 
        assume (not (wall_in_board board wall));
        try remove_wall board wall; false
        with Wall_out_of_bounds -> true 
      end  
