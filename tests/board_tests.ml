...
let tests =
  [ Test.make ~name:"wall_in_board_imp_pos_in_board"
      ~print:(Print.pair wall_print board_print) (Gen.pair wall_gen board_gen)
      (fun (wall, board) ->
  ...
  ; Test.make ~name:"add_wall_not_in_board"
      ...
      (fun (wall, board) ->
        ...
        try add_wall board wall ; false with Wall_out_of_bounds -> true )
  ; Test.make ~name:"remove_wall_not_in_board"
      ~print:(Print.pair wall_print board_print) (Gen.pair wall_gen board_gen)
      (fun (wall, board) ->
        assume (not (wall_in_board board wall)) ;
        try remove_wall board wall ; false with Wall_out_of_bounds -> true )
  ; Test.make ~name:"add_wall" ~print:(Print.pair wall_print board_print)
      (Gen.pair wall_gen board_gen) (fun (wall, board) ->
        assume (wall_in_board board wall) ;
        add_wall board wall ;
        true )
  ; Test.make ~name:"remove_wall" ~print:(Print.pair wall_print board_print)
      (Gen.pair wall_gen board_gen) (fun (wall, board) ->
        assume (wall_in_board board wall) ;
        try remove_wall board wall ; false with Wall_missing -> true )
  ; Test.make ~name:"add_remove_wall" ~print:(Print.pair wall_print board_print)
      (Gen.pair wall_gen board_gen) (fun (wall, board) ->
        assume (wall_in_board board wall) ;
        add_wall board wall ;
        remove_wall board wall ;
        true )
  ; Test.make ~name:"add_add_wall" ~print:(Print.pair wall_print board_print)
      (Gen.pair wall_gen board_gen) (fun (wall, board) ->
        assume (wall_in_board board wall) ;
        try add_wall board wall ; add_wall board wall ; false
